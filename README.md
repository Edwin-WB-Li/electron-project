...
- contextBridge.exposeInMainWorld 将 Electron 的 ipcRenderer 功能安全地暴露给渲染进程，允许渲染进程与主进程进行通信,这样渲染进程可以通过 window.xxx 访问这些方法（名字可以自定义）
...
![流程模型](https://i-blog.csdnimg.cn/direct/e30957e6aec44ac789aa2bef9995a4d2.jpeg#pic_center)
...
- ipcMain (在主进程中(main.ts)使用)

- ipcRenderer (在渲染进程或 preload 脚本中使用)

- contextBridge (在 preload 脚本中使用，用于安全暴露 ipcRenderer API)
